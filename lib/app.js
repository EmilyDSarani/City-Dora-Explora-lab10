...
const { mungeLocationData, mungeWeatherData } = require ('./utils');
...
app.get('/weather', async(req, res) => {
  try {
    const cityWeather = req.query.search;
   
    const response = await request.get(`https://us1.locationiq.com/v1/search.php?key=${process.env.WEATHER_KEY}&q=${cityWeather}&format=json`);
   
    const mungeResponseW = mungeWeatherData(response.body);
